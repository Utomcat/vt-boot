...
import com.ranyk.vt.boot.example.web.freamwork.repository.dict.DictTypeRepository;
...
public class DictTypeService extends ServiceImpl<DictTypeRepository, DictType> {
    ...
    public void saveOneDictType(DictTypeDTO dictTypeDTO) {
        ...
        // 数据转换
        DictType dictType = dictMapper.dictTypeDTOToDictType(dictTypeDTO);
    ...
    public void updateOneDictType(DictTypeDTO dictTypeDTO) {
        ...
        if (StrUtil.isNotBlank(dictTypeDTO.getName())){
            Integer countByName = this.dictTypeRepository.selectCountByNameEqAndIdNotEq(dictTypeDTO.getName(), dictType.getId());
            if (countByName > 0) {
                log.error("更新字典类型时, 字典类型名称 {} 已存在!", dictTypeDTO.getName());
                throw new ServiceException("更新字典类型时, 字典类型名称 %s 已存在!".formatted(dictTypeDTO.getName()));
            }
            dictType.setName(dictTypeDTO.getName());
        }
        if (StrUtil.isNotBlank(dictTypeDTO.getCode())) {
            Integer countByCode = this.dictTypeRepository.selectCountByCodeEqAndIdNotEq(dictTypeDTO.getCode(), dictType.getId());
            if (countByCode > 0) {
                log.error("更新字典类型时, 字典类型编码 {} 已存在!", dictTypeDTO.getCode());
                throw new ServiceException("更新字典类型时, 字典类型编码 %s 已存在!".formatted(dictTypeDTO.getCode()));
            }
    ...
    public PageResponse<DictTypeDTO> queryDictTypeList(DictTypeDTO dictTypeDTO) {
        ...
        return PageUtils.buildPageResponse(list, dictMapper.dictTypeListToDictTypeDTOList(list.getRecords()));
    }
    ...
    private void verifyDictTypeParams(DictTypeDTO dictTypeDTO, OperateTypeEnum operateType) {
        switch (operateType) {
            ...
            case BATCH_DELETE -> verifyBatchDeleteDictTypeParams(dictTypeDTO);
    ...
    /**
     * 字典类型参数校验 - 删除单条字典类型数据参数验证
     *
     * @param dictTypeDTO 字典类型 DTO 对象, {@link DictTypeDTO}
     */
    private void verifyDeleteDictTypeParams(DictTypeDTO dictTypeDTO) {
        if (StrUtil.isBlank(dictTypeDTO.getId())) {
            log.error("删除字典类型失败，数据 ID 不能为空!");
            throw new ServiceException("删除字典类型失败，数据 ID 不能为空!");
        }
    }

    /**
     * 字典类型参数校验 - 批量删除字典类型数据参数验证
     *
     * @param dictTypeDTO 字典类型 DTO 列表, {@link DictTypeDTO}
     */
    private void verifyBatchDeleteDictTypeParams(DictTypeDTO dictTypeDTO) {
        if (CollectionUtil.isEmpty(dictTypeDTO.getIds())) {
            log.error("批量删除字典类型失败，字典类型 ID 列表不能为空!");
            throw new ServiceException("批量删除字典类型失败，字典类型 ID 列表不能为空!");
        }
    }

    /**
     * 字典类型参数校验 - 修改单条字典类型数据参数验证
     *
     * @param dictTypeDTO 字典类型 DTO 对象, {@link DictTypeDTO}
     */
