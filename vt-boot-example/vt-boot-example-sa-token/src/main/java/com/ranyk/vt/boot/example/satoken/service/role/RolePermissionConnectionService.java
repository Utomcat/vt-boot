...
import com.ranyk.vt.boot.example.satoken.repository.role.RolePermissionConnectionRepository;
...
public class RolePermissionConnectionService extends ServiceImpl<RolePermissionConnectionRepository, RolePermissionConnection> {
    ...
    public List<RolePermissionConnectionDTO> queryRolePermissionConnectionByRoleIds(RolePermissionConnectionDTO rolePermissionConnectionDTO) {
        ...
        return roleMapper.rolePermissionConnectionEntityListToRolePermissionConnectionDTOList(Optional.of(rolePermissionConnections.stream().filter(Objects::nonNull).collect(Collectors.toList())).orElse(Collections.emptyList()));
    }
